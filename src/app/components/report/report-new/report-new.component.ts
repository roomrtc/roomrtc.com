...
export class ReportNewComponent implements OnInit {
  ...
  Demo = [
    {
      ...
      parent: {id: 2, ten_hang_muc: 'Thống kê bệnh nhân'}
    },
    {
      id: 20,
  ...
  types = [{name: 'Kieu 1', id: 1}, {name: 'Kieu 2', id: 2}];
  selectedValueType = [];
  ...
  constructor(private fb: FormBuilder, private reportServices: ReportService,
              private categoryServices: CategoryService) {
    this.createForm();
    this.categoryServices.getDataSelect().subscribe(
        (data: any) => { this.DuLieuSelect = data; });
  }
  ...
  handleAdd(event) {
    ...
    if (event.id) {
      ...
      const check = _.findIndex(this.DanhSachSelected, function(o) {
      ...
      const temp = this.DuLieuSelect.filter((element) => {
                                      return element.parent.ten_hang_muc ===
                                             event.ten_hang_muc;
                                    })
                       .map((result) => {
                         const newCategory = new Category();
  ...
  handleRemove(event) {
    ...
    if (value.id) {
      ...
      console.log(event);
      const tmp = this.reportForm.controls['select_hang_muc'].value;
  ...
  deleteChild(hangMuc) {
    console.log(hangMuc);
    // Find hang muc cha
    const temp = this.DanhSachSelected.find((element) => {
      return element.ten_hang_muc === hangMuc.parent.ten_hang_muc;
    });
    console.log(temp);
    _.remove(temp.danh_sach_hang_muc,
             (current: any) => current.id === hangMuc.id);
    // Xet truong hop da xoa het item trong mot nhom hang muc thi xoa luon
    // ca nhom hang muc
    console.log(temp);
    if (!temp.danh_sach_hang_muc.length) {
      _.remove(this.DanhSachSelected,
               (current) => current.ten_hang_muc === temp.ten_hang_muc);
    }
  }
  // Xu li su kien nguoi dung click vao dau x de xoa hang muc
  handleDeleteHangMucControl(value, idGroup) {
    // this.selectedHangMuc=[{ id: 19, ten_hang_muc: "Tổng số khám", group: {
    // id: 2, ten_hang_muc: "Thống kê bệnh nhân" } }]
    // Lay danh sach selected trong controll
    const select_hang_mucControll =
        this.reportForm.controls['select_hang_muc'].value;
    // Xoa hang muc selected
    _.remove(select_hang_mucControll,
             (current: any) => current.id === value.id);
    this.reportForm.controls['select_hang_muc'].setValue(
        select_hang_mucControll);
    // Find hang muc cha
    const temp =
        this.DanhSachSelected.find((element: any) => element.id === idGroup);
    _.remove(temp.danh_sach_hang_muc,
             (current: any) => current.id === value.id);
    // Xet truong hop da xoa het item trong mot nhom hang muc thi xoa
    // luon ca nhom hang muc
    if (!temp.danh_sach_hang_muc.length) {
      _.remove(this.DanhSachSelected,
               (current: any) => current.ten_hang_muc === temp.ten_hang_muc);
      // Xoa luon tag group
  ...
  onSubmit() {
    console.log('submit');
    const prepearModel = this.reportForm.value;
    this.reportForm.reset();
    this.isLoading = true;
    prepearModel.select_hang_muc = this.DanhSachSelected;
    this.reportServices.saveItem(prepearModel)
